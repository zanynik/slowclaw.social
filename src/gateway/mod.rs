...
// AppState, LocalModelDownloadJob, LocalModelRuntimeState, LocalModelRuntimeSnapshot, OpenRouterOAuthSession, OpenRouterOAuthStatus, JournalTranscriptionJob, TranscriptionModelCacheEntry, TRANSCRIPTION_MODEL_CACHE, TRANSCRIPTION_MODEL_CACHE_TTL_SECS moved to state.rs
...
pub(crate) fn local_media_capabilities(config: &Config) -> MediaToolCapabilities {
...
pub(crate) fn reset_workspace_synthesizer_status_for_provider_change(workspace_dir: &StdPath) {
    let mut next = workspace_synthesizer::load_status(workspace_dir);
...
pub(crate) fn pairing_auth_error(
    state: &AppState,
    headers: &HeaderMap,
    scope: &str,
...
pub(crate) fn available_local_transcription_models() -> Vec<String> {
