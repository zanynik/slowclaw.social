...
Examples:
  ...
  slowclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  slowclaw cron add-every 60000 'Ping heartbeat'     # every minute
  slowclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  slowclaw cron once 2h 'Follow up on deployment'
  slowclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  slowclaw cron update <task-id> --expression '0 8 * * *'
  slowclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  slowclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  slowclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  slowclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash SlowClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash SlowClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  slowclaw peripheral flash
  slowclaw peripheral flash --port /dev/cu.usbmodem12345
  slowclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash SlowClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
