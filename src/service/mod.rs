...
const SERVICE_LABEL: &str = "com.slowclaw.daemon";
const WINDOWS_TASK_NAME: &str = "SlowClaw Daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "slowclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["slowclaw", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "slowclaw.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["slowclaw", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "slowclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["slowclaw", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["slowclaw", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/slowclaw");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/slowclaw");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "slowclaw", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove slowclaw from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/slowclaw)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: slowclaw service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    let exe = std::env::current_exe().context("Failed to resolve current executable")?;
    let unit = format!(
        "[Unit]\nDescription=SlowClaw daemon\nAfter=network.target\n\n[Service]\nType=simple\nExecStart={} daemon\nRestart=always\nRestartSec=3\n\n[Install]\nWantedBy=default.target\n",
        exe.display()
    );

    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "slowclaw.service"]));
...
/// Check if the slowclaw user exists and has expected properties.
...
fn check_slowclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "slowclaw"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser slowclaw && delgroup slowclaw",
            "addgroup -S slowclaw && adduser -S -s /sbin/nologin -H -D -G slowclaw slowclaw",
        )
    } else {
        ("userdel slowclaw", "useradd -r -s /sbin/nologin slowclaw")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'slowclaw' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'slowclaw' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/slowclaw" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: slowclaw user has home directory '{}' (expected /var/lib/slowclaw or /nonexistent)",
                        home
                    );
                }
...
fn ensure_slowclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "slowclaw"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_slowclaw_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "slowclaw"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "slowclaw"])
                .output()
                .context("Failed to create slowclaw group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create slowclaw group: {}", stderr.trim());
            }
            println!("✅ Created system group: slowclaw");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create slowclaw user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create slowclaw user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "slowclaw"])
            .output()
            .context("Failed to create slowclaw user")?;
    ...
    println!("✅ Created system user: slowclaw");
    Ok(())
}

/// Change ownership of a path to slowclaw:slowclaw
#[cfg(unix)]
fn chown_to_slowclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["slowclaw:slowclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to slowclaw:slowclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_slowclaw(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_slowclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "slowclaw:slowclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to slowclaw:slowclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_slowclaw(_path: &Path) -> Result<()> {
    Ok(())
}
...
/// prefer-`~/.slowclaw` / fallback-`~/.slowclaw` precedence as
...
fn data_dir_under_home(home: PathBuf) -> PathBuf {
    ...
    let legacy_dir = home.join(".slowclaw");
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'slowclaw' cannot write {} ({details}). \
             Re-run `sudo slowclaw service install` and ensure ownership is slowclaw:slowclaw.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/slowclaw",
            exe_path.display(),
            exe_path.display()
        );
    }
...
command_user="slowclaw:slowclaw"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/slowclaw/access.log"
error_log="/var/log/slowclaw/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/slowclaw");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo slowclaw service install"
        );
    }

    ensure_slowclaw_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/slowclaw");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/slowclaw");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_slowclaw(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by slowclaw:slowclaw)",
    ...
    chown_recursive_to_slowclaw(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/slowclaw");
    ...
    run_checked(Command::new("rc-update").args(["add", "slowclaw", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/slowclaw");
    println!("   Config path: /etc/slowclaw/config.toml");
    println!("   Start with: sudo slowclaw service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("slowclaw-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/slowclaw.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "SlowClaw Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/slowclaw");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/slowclaw"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"slowclaw\""));
        assert!(script.contains("description=\"SlowClaw daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/slowclaw\""));
        assert!(script.contains("command_args=\"--config-dir /etc/slowclaw daemon\""));
        ...
        assert!(script.contains("command_user=\"slowclaw:slowclaw\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/slowclaw/access.log\""));
        assert!(script.contains("error_log=\"/var/log/slowclaw/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/slowclaw");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/slowclaw");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/slowclaw");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/slowclaw"), true);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/slowclaw/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/slowclaw/workspace'".to_string(),
                "slowclaw".to_string()
            ]
        );
    }
