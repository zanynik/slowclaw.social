...
function App() {
  ...
  async function uploadJournalFile(file: File, kind: "audio" | "video") {
    ...
    if (!token) {
      if (isDesktopClient) {
        ...
        setRecordingHint(mobileGatewayBlockedMessage(mobileHostReachability, "Upload"));
        return;
      }
    }
  ...
  async function saveJournalTextDraft() {
    ...
    if (!token && !isDesktopClient) {
      setJournalSaveStatus(mobileGatewayBlockedMessage(mobileHostReachability, "Save"));
      return;
    }
  ...
  async function transcribeSelectedJournalMedia() {
    ...
    if (!token && !isDesktopClient) {
      setJournalSaveStatus(mobileGatewayBlockedMessage(mobileHostReachability, "Transcription"));
      return;
    }
    ...
    try {
      ...
      if (!token) {
        if (isDesktopClient) {
          ...
          setChatStatus(mobileGatewayBlockedMessage(mobileHostReachability, "Chat"));
          return;
        }
      }
      let threadId = chatThreadId.trim();
  ...
  async function loadRuntimeConfigForSettings() {
    ...
    if (isDesktopClient) {
      ...
      try {
        ...
        if (setupStatus?.availableModels?.length) {
          for (const modelName of setupStatus.availableModels) {
  ...
  const mobileDesktopHostOffline =
    !isDesktopClient &&
    !needsMobileQrLogin &&
    mobileHostReachability === "offline";
