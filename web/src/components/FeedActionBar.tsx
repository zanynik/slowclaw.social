...
export interface FeedActionBarProps {
  ...
  /** Called when the repost icon is tapped (parent performs the protocol repost). */
  onRepost?: () => void;
  /** Whether this post is currently optimistically reposted (fills the icon). */
  reposted?: boolean;
  /** True while a repost is in flight (shows … on the badge). */
...
export function FeedActionBar({
  ...
  return (
    <div className="feed-action-bar" role="group" aria-label="Post actions">
      ...
      {/* Repost: real action when onRepost is wired, else deep-link/count display. */}
      {onRepost ? (
        <button
          type="button"
          className={`feed-action${reposted ? " reposted" : ""}`}
          onClick={onRepost}
        ...
        >
          ...
          <span className="feed-action-count">{repostLoading ? "…" : ((repostCount ?? 0) + (reposted ? 1 : 0)) > 0 ? ((repostCount ?? 0) + (reposted ? 1 : 0)) : ""}</span>
        </button>
      ) : permalink ? (
        <a
